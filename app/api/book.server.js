...
export async function detailMyCharacter(childAccessToken, characterId) {
	try {
		const response = await fetch(
			`${API_BASE_URL}/api/characters/${characterId}?page=0&size=20&sort=name`,
			{
				method: 'GET',
				headers: {
					'Authorization': `Bearer ${childAccessToken}`
...
export async function readingStoryPage(childAccessToken, storyId, pageNum) {
	try {
		const response = await fetch(
...
export async function readingStoryAllPage(childAccessToken, storyId) {
	try {
		const response = await fetch(
			`${API_BASE_URL}/api/stories/${storyId}/scenes`,
			{
				method: 'GET',
				headers: {
					'Authorization': `Bearer ${childAccessToken}`
